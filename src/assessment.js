...
*/
function async() {
  return new Promise($q => setTimeout((changed) => {$q(true)
  } , 1));
}
...
//
var context1 = sum.bind(contextObj);
...
//
var context2 = sum.bind(contextObj)
...
function partyTime (partyName){
  var guestList = [];
  return function addGuest (guests){
...
function compareValues (val,param){
  if (val === param){
     return "Exact match";}
  if (val == param){
